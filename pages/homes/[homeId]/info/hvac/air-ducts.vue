...
const pageEdit = ref()
const page_title = ref("Air Ducts")
const project_type = ref("air_ducts_inspection")
...
onMounted(() => {
    ...
    getDoc(docRef).then((docSnap) => {
        if (docSnap.exists()) {
            ...
            if (property.info && property.info.hvac && property.info.hvac.airDucts) {
                pageSource.value = property.info.hvac.airDucts
            }
            else {
            pageSource.value = {
                ...pageTemplate.value
        ...
        <article class="p-4 bg-white shadow-md rounded-md">
            <div class="space-y-3 relative p-4 rounded-lg hover:bg-gray-50 cursor-pointer" @click="openBasicEditModal">
                ...
                <div class="space-y-2">
                    ...
                    <p class="font-medium">Last cleaning date</p>
                    <p class="text-gray-500">
                        {{ pageSource.basicInformation.lastCleaningDate }}
                    </p>
                </div>
            </div>
        </article>
        ...
        <div class="flex flex-col h-full">
            ...
            <div v-if="editType === 'basic'" class="flex-1 p-4 overflow-y-auto space-y-4">
                ...
                <div class="space-y-4">
                    ...
                    <input 
                                type="date" 
                                v-model="pageEdit.basicInformation.lastCleaningDate"
                                class="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    placeholder="Date air ducts were last cleaned"
                            />
                </div>
            </div>
