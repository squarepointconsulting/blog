...
const pageEdit = ref()
const page_title = ref("Fireplace")
const project_type = ref("fireplace_inspection")
...
onMounted(() => {
    ...
    getDoc(docRef).then((docSnap) => {
        if (docSnap.exists()) {
            ...
            if (property.info && property.info.interior && property.info.interior.fireplace) {
                pageSource.value = property.info.interior.fireplace
            }
            else {
            pageSource.value = {
                ...pageTemplate.value
        ...
        <article class="p-4 bg-white shadow-md rounded-md">
            <div class="space-y-3 relative p-4 rounded-lg hover:bg-gray-50 cursor-pointer" @click="openBasicEditModal">
                ...
                <div class="space-y-2">
                    ...
                    <p class="font-medium">Number of fireplaces</p>
                    <p class="text-gray-500">
                        {{ pageSource.basicInformation.numberOfFireplaces }}
                    </p>
                </div>
            </div>
        </article>
        <article class="p-4 bg-white shadow-md rounded-md">
            <div class="space-y-3 relative p-4 rounded-lg hover:bg-gray-50 cursor-pointer"
                ...
                <div class="space-y-2">
                    ...
                    <p class="font-medium">Last serviced</p>
                    <p class="text-gray-500">
                        {{ pageSource.detailedInformation.lastServiced }}
                    </p>
                </div>
            </div>
        </article>
        ...
        <div class="flex flex-col h-full">
            ...
            <div v-if="editType === 'basic'" class="flex-1 p-4 overflow-y-auto space-y-4">
                <div class="space-y-4">
                    ...
                    <div class="space-y-3">
                        <div v-for="option in ['Electric', 'Gas', 'Pellet', 'Wood']"
                            :key="option" :class="[
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-3">
                        <div v-for="option in ['1', '2', '3', 'More']"
                            :key="option" :class="[
                                'flex items-center justify-between p-4 rounded-lg border',
                                pageEdit.basicInformation.numberOfFireplaces === option ? 'border-blue-500' : 'border-gray-200'
                            ]" @click="pageEdit.basicInformation.numberOfFireplaces = option">
                            <span class="text-base">{{ option }}</span>
                            <div :class="[
                                'w-6 h-6 rounded-full border-2 flex items-center justify-center',
                                pageEdit.basicInformation.numberOfFireplaces === option ? 'border-blue-500' : 'border-gray-300'
                            ]">
                                <div v-if="pageEdit.basicInformation.numberOfFireplaces === option"
                                    class="w-3 h-3 rounded-full bg-blue-500" />
                            </div>
                        </div>
                    </div>
            ...
            <div v-else class="flex-1 p-4 overflow-y-auto space-y-4">
                <div class="space-y-4">
                    ...
                    <input 
                                type="date" 
                                v-model="pageEdit.detailedInformation.lastServiced"
                                class="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    placeholder="Date fireplace was last serviced"
                            />
                </div>
            </div>
