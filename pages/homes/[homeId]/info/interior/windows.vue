...
const pageEdit = ref()
const page_title = ref("Windows")
const project_type = ref("windows_inspection")
...
onMounted(() => {
    ...
    getDoc(docRef).then((docSnap) => {
        if (docSnap.exists()) {
            ...
            if (property.info && property.info.interior && property.info.interior.windows) {
                pageSource.value = property.info.interior.windows
            }
            else {
            pageSource.value = {
                ...pageTemplate.value
        ...
        <article class="p-4 bg-white shadow-md rounded-md">
            <div class="space-y-3 relative p-4 rounded-lg hover:bg-gray-50 cursor-pointer" @click="openBasicEditModal">
                ...
                <div class="space-y-2">
                    ...
                    <p class="font-medium">Number of windows</p>
                    <p class="text-gray-500">
                        {{ pageSource.basicInformation.numberOfWindows }}
                    </p>
                </div>
            </div>
        </article>
        <article class="p-4 bg-white shadow-md rounded-md">
            <div class="space-y-3 relative p-4 rounded-lg hover:bg-gray-50 cursor-pointer"
                ...
                <div class="space-y-2">
                    ...
                    <p class="font-medium">Last cleaned</p>
                    <p class="text-gray-500">
                        {{ pageSource.detailedInformation.lastCleaned }}
                    </p>
                </div>
            </div>
        </article>
        ...
        <div class="flex flex-col h-full">
            ...
            <div v-if="editType === 'basic'" class="flex-1 p-4 overflow-y-auto space-y-4">
                ...
                <div class="space-y-4">
                    ...
                    <input 
                                type="number" 
                                v-model="pageEdit.basicInformation.numberOfWindows"
                                class="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div v-else class="flex-1 p-4 overflow-y-auto space-y-4">
                <div class="space-y-4">
                    ...
                    <div class="space-y-3">
                        <div v-for="option in ['Champion', 'Castle', 'Jeld-Wen', 'Simpson', 'Anderson', 'Marvin', 'Harvey', 'Milgard', 'Pella', 'Other']" :key="option" :class="[
                            ...
                            <div :class="[
                                ...
                                <div v-if="pageEdit.detailedInformation.brand === option"
                ...
                <div class="space-y-4">
                    ...
                    <input 
                                type="date" 
                                v-model="pageEdit.detailedInformation.lastCleaned"
                                class="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    placeholder="Date windows were last cleaned"
                            />
                </div>
            </div>
